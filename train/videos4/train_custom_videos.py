...
from models.vq_vae.vq_vae1_video import VqVae1 as VqVae
from train.train_utils import get_model_size, save_checkpoint, AverageMeter, ProgressMeter, NormalizeInverseDummy, \
    NormalizeDummy, train_visualize, save_images
...
class TrainVqVae:
    ...
    def train(self):
        ...
        meter_loss_constr1 = AverageMeter('Constr1', ':6.5f')
        meter_loss_constr2 = AverageMeter('Constr2', ':6.5f')
        meter_loss_perp = AverageMeter('Perplexity', ':6.2f')
        progress = ProgressMeter(
            self.training_loader.epoch_size()['__Video_0'],
            [batch_time, data_time, meter_loss, meter_loss_constr1, meter_loss_constr2, meter_loss_perp],
        ...
        for i in range(self.start_steps, self.num_steps):
            ...
            recon_error1 = F.mse_loss(images_recon[:, 0], images[:, 0])
            recon_error2 = 10 * F.mse_loss(images_recon[:, 1:], images[:, 1:])
            loss = recon_error1 + recon_error2 + vq_loss
            loss.backward()

            self.optimizer.step()

            meter_loss_constr1.update(recon_error1.item(), 1)
            meter_loss_constr2.update(recon_error2.item(), 1)
            ...
            if i % 200 == 0:
                ...
                images, images_recon = map(lambda t: (1.0 + torch.cumsum(t, 1)) / 2.0, [images, images_recon])
                ...
                if self.run_wandb:
                    logs = {
                        'iter': i,
                        'loss_recs1': meter_loss_constr1.val,
                        'loss_recs2': meter_loss_constr2.val,
                        'loss': meter_loss.val,
