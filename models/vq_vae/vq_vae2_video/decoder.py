...
class DecoderBlock3d(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        self.res_path = nn.Sequential(OrderedDict([
            ...
            ('relu_2', nn.ReLU()),
            ('conv_2', nn.Conv2d(in_channels=self.n_hid, out_channels=self.n_hid, kernel_size=7, padding=3)),
            ('relu_3', nn.ReLU()),
            ('conv_3', nn.Conv2d(in_channels=self.n_hid, out_channels=self.n_hid, kernel_size=7, padding=3)),
            ('relu_4', nn.ReLU()),
            ('conv_4', nn.Conv2d(in_channels=self.n_hid, out_channels=self.n_out, kernel_size=7, padding=3))]))
...
class Decoder2(nn.Module):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        # decode spatially
        # (b, n_hid, nd, nh, nw) -> (b, d, c, h, w)
        # (b, 1024, 4, 16, 16) -> (b, 32, 512, 32, 32)
        decode_blks = []
        n_prev = self.n_init
